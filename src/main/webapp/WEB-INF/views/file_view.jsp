...
<t:header title=".">
    <script>
        ...
        function update(path, data = null) {
            return fetch("<c:url value="/v/${fileId}/"/>" + path, {
                method: "POST",
                body: data
            })
        }
...
<body class="centered-container-scrollable" style="gap: 20px">
<h3 id="status"></h3>
<input onfocusout="updateFilename()" id="filename" placeholder="Filename" value="${filename}">
...
<c:if test="${contentType.startsWith('image')}">
    <img id="image" alt="${filename}" src="<c:url value="/d/${fileId}/${filename}"/>">
</c:if>
