      ...
      if (argClass.isRecord()) {
        ...
        for (RecordComponent component : components) {
          ...
          if (type == UUID.class && component.getAnnotation(Id.class) != null) {
            ...
            params.addValue(component.getName(), value,
                Optional.ofNullable(value != null ? sqlTypes.get(value.getClass()) : null).orElse(Types.OTHER));
          }
        }
      } else {
        var paramName = method.getParameters()[argIndex].getName();
        params.addValue(paramName, argCurrent,
            Optional.ofNullable(sqlTypes.get(argCurrent.getClass())).orElse(Types.OTHER));
      }
    }
    return id;
  }
