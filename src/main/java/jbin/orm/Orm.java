...
public class Orm {
    ...
    private String getQuerySql(String tableName, Method method) {
        ...
        if (select != null) return "select * from " + tableName + " " + select.value();
        var upsert = method.getAnnotation(Insert.class);
        if (upsert != null) return "insert into " + tableName + " " + upsert.value();
        var delete = method.getAnnotation(Delete.class);
        if (delete != null) return "delete from " + tableName + " " + delete.value();
        return null;
    }

    private Object processResult(Method method, Class<?> returnType, PreparedStatement statement, UUID id) throws SQLException, InvocationTargetException, InstantiationException, IllegalAccessException {
    ...
    private UUID fillStatementAndGetUUID(Object[] args, UUID id, PreparedStatement statement) throws IllegalAccessException, InvocationTargetException, SQLException {
        int count = 1;
    ...
    private Object objectFromResult(ResultSet result, Class<?> returnType) throws SQLException, InvocationTargetException, InstantiationException, IllegalAccessException {
    ...
    private void setAutoStatement(PreparedStatement statement, int count, Object value, Class<?> valueType) throws SQLException {
